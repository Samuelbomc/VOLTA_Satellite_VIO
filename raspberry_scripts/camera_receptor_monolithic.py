...
class TelemetryPublisher:
    def publish_imu(self, packet: ImuPacket):
        logging.debug("IMU publish placeholder: %s", packet)  # TEMPORARY

    def publish_frame(self, frame: CameraFrame):
        logging.debug("Frame publish placeholder: %s", frame)  # TEMPORARY
...
class TelemetryProcessor:
    ...
    def handle_imu_packet(self, recv_time: float, data: tuple):
        ...
        self.publisher.publish_imu(packet)
...
class CameraTelemetryNode:
    ...
    def read_exact_bytes(self, n_bytes: int, max_time: float = READ_MAX_TIME) -> bytes:
        ...
        while len(data) < n_bytes:
            if (time.monotonic() - start_time) > max_time:
                logging.debug(
                    "Read timeout after %.3fs while waiting for %d bytes (received %d)",
                    max_time,
                    n_bytes,
                    len(data),
                )  # TEMPORARY
    ...
    def handle_buffer_overflow(self):
        ...
        if in_waiting > 128:
            ...
            logging.debug(
                "Minor desync detected. Flushing 1 byte from buffer (%d bytes waiting).", in_waiting
            )  # TEMPORARY
    ...
    def listen_and_decode(self) -> str:
        ...
        if not self.init_received and packet_id != ID_INIT_CMD:
            logging.debug(
                "Waiting for INIT. Ignoring packet id=0x%02X len=%d", packet_id, payload_len
            )  # TEMPORARY
        ...
        if self.calculate_crc16(data_for_crc) != received_crc:
            logging.debug(
                "CRC mismatch for packet id=0x%02X (expected=0x%04X received=0x%04X)",
                packet_id,
                self.calculate_crc16(data_for_crc),
                received_crc,
            )  # TEMPORARY
            return "ERROR_CRC"
    ...
    try:
        ...
        while telemetry_node.is_running:
            ...
            if time_since_last_imu > 1:
                logging.error(
                    "WATCHDOG ALARM: No IMU data in %.1fs. Triggering hardware reset.",
                    time_since_last_imu,
                )
